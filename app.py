...
def run_forecast_flow(cfg, fh_selection, id_selection, show_actuals):
    with st.spinner(
        "Model is training and forecasts are being made, please wait..."
    ):
        make_dataset(cfg)
        model_version = build_features(cfg, forecast_horizon=fh_selection)
        train_model(
            cfg, model_version=model_version, forecast_horizon=fh_selection
        )
        predict_model(
            cfg, model_version=model_version, forecast_horizon=fh_selection
        )
        ...
        st.markdown(
            "<h3 style='text-align: center;'>Forecast Results</h3>",
...
def render_forecast_tab(cfg):
    ...
    with params_container:
        ...
        with col1:
            fh_selection = st.select_slider(
                "Select forecast length in days:",
                options=fh_options,
                value=fh_options[0],
            )
        ...
        with col2:
            id_selection = st.selectbox(
                "Select family-store pair ID:", id_options
            )
        ...
        if show_actuals:
            st.success(
                "Actual sales for the forecast period will be shown!",
                icon="📈",
            )
...
def render_eda_tab():
    ...
    daily["smooth7_sales"] = (
        daily["sales"].rolling(window=7, center=True, min_periods=3).mean()
    )
    daily["smooth30_sales"] = (
        daily["sales"].rolling(window=30, center=True, min_periods=15).mean()
    )
    daily["smooth365_sales"] = (
        daily["sales"].rolling(window=365, center=True, min_periods=183).mean()
    )

    fig = go.Figure()
    fig.add_trace(
        go.Scatter(
            x=daily.index, y=daily["sales"], mode="lines", name="Daily Sales"
        )
    )
    fig.add_trace(
        go.Scatter(
            x=daily.index,
            y=daily["smooth7_sales"],
            mode="lines",
    ...
    fig.update_layout(
        ...
        xaxis_title="Date",
        yaxis_title="Sales (Qty)",
    )
    st.plotly_chart(fig)

    yoy = (
        daily.groupby(["year", "day_of_year"])
        .agg({"smooth7_sales": "sum"})
        .reset_index()
    )
    fig = px.line(yoy, x="day_of_year", y="smooth7_sales", color="year")
    fig.update_layout(
        title="YoY Smoothed Sales Plot",
    ...
    fig.update_layout(
        title="Smoothed Sales Over Time by Family",
        xaxis_title="Date",
        yaxis_title="Sales (Qty)",
    )
    st.plotly_chart(fig)
...
def main():
    ...
    st.sidebar.markdown(
        '<p class="big-font">Navigate</p>', unsafe_allow_html=True
    )
    tab = st.sidebar.radio("", tabs)

    if tab == "Forecast Tool":
        st.markdown(
            "<h1 style='text-align: center;'>Chain Level Forecast</h1>",
            unsafe_allow_html=True,
        )
        st.write(
            "The section provides an interactive platform for generating sales forecasts across various family-store pairings within the retail chain.",
            unsafe_allow_html=True,
        )
        ...
        **Here’s how one can leverage this tool:**
        ...
        st.info(
            """It's important to note that the dashboards presented are just a part of the exploratory analysis. They've been selected to offer useful insights without overwhelming users with too much information.""",
            icon="📊",
        )
        render_eda_tab()
