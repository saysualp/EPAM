...
def make_dataset(cfg: DictConfig):
    ...
    final_df['dcoilwtico'] = final_df['dcoilwtico'].fillna(method=cfg.make_dataset.fillna_method)
    ...
    for rule in cfg.make_dataset.remove_unusual_data:
        ...
        final_df = final_df[~((final_df['family'].isin(families)) & (final_df['date'] <= before_date))]

    # Create the unique identifier for time series
    group_by = cfg.make_dataset.group_by
    final_df['id'] = final_df[group_by].apply(lambda row: '_'.join(row.values.astype(str)), axis=1)
