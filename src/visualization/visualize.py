...
def visualize(cfg: DictConfig, id: int, model_version: str, show_actuals = False):
    """
    Visualizes the output comparing with history (last 365 days, if exists) and future
    """
    ...
    # History
    history_dates = y_train[id].time_index[len(y_train[id].time_index)-365:len(y_train[id].time_index)]
    history_values = y_train[id].values()[len(y_train[id].values())-365:len(y_train[id].values())]
