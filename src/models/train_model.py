...
def train_model(cfg: DictConfig, forecast_horizon: int, model_version: str):
    ...
    Trains model
    """
    logger.info(f'Starting the training process for model version {model_version}.')
    ...
    future_cov_holdout_path = output_dir / f"future_cov_holdout.pkl"
    ...
    y_holdout = TimeSeries.from_group_dataframe(holdout,
                                          group_cols='id',
                                          time_col='date',
                                          static_cols=static_cov_cols,
                                          value_cols=['sales'],
    ...
    future_cov_holdout = TimeSeries.from_group_dataframe(holdout,
                                            group_cols='id',
                                            time_col='date',
                                            static_cols=static_cov_cols,
                                            value_cols=future_cov_cols,
    ...
    model = LightGBMModel(lags=list(cfg.train.target_lags), 
    ...
    logger.info(f"Future holdout covariates saved to {future_cov_holdout_path}")

    logger.info(f'Model and associated data saved for model version {model_version}.')
